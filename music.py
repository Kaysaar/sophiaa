...
with open("./Secret/password")as pas:
    LAVALINK_PASSWORD=pas.read()
...
async def _join(ctx: lightbulb.Context) -> Optional[hikari.Snowflake]:
    ...
    if not voice_state:
        await ctx.respond("Najpierw musisz być na kanale glosowym.")
        return None
    ...
    if HIKARI_VOICE:
        ...
        try:
            ...
            await ctx.respond(
                "Nie jestem w stanie dolaczyc do kanalu."
            )
            return None
...
async def start_lavalink(event: hikari.ShardReadyEvent) -> None:
    builder = (
        lavasnek_rs.LavalinkBuilder(event.my_user.id, _token)
...
async def play(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await ctx.respond(f"Sophia doradza abys uzyl komendy `/join` first")
        return
